...
function Login() {
    ...
    const getAxiosErrorMessage = (err) => {
        ...
        if (typeof asText === 'string') {
            if (asText.includes("Unable to resolve service for type 'GoogleAuthService'")) {
                return 'Backend lỗi cấu hình DI: chưa register GoogleAuthService.';
            }
            if (asText.includes('unregistered_origin') || asText.includes('The given origin is not allowed')) {
            ...
            try {
                if (notification.isNotDisplayed()) {
                    ...
                    const map = {
                        ...
                        opt_out_or_no_session: 'Bạn chưa đăng nhập Google trong trình duyệt.',
                        suppressed_by_user: 'Đăng nhập Google bị chặn bởi người dùng/trình duyệt.',
                        unregistered_origin: 'Google OAuth: origin chưa được phép (Authorized JavaScript origins).',
